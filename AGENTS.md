...
- **Linux build deps**: `libgtk-3-dev` + `libwebkit2gtk-4.1-dev`. Build with `-tags webkit2_41` (`wails build -tags webkit2_41`). Default Wails build wants `webkit2gtk-4.0`, but `libwebkit2gtk-4.0-dev` was dropped on modern Ubuntu (24.04+) and GitHub runners - missing `webkit2gtk-4.0.pc` causes `pkg-config` failures (see `.github/workflows/release.yml`, `Makefile build-linux`).
...
- Frameless: `Frameless: true` in `wails.Run` options (`main.go`). Native OS chrome removed on all platforms.
- Custom titlebar: `frontend/src/lib/components/TitleBar.svelte` (32px, `fixed` top bar, replaces the old `Navbar`). Left: drag region (app icon, `t("window_title")`, version badge linking to releases with `--wails-draggable: no-drag`). Right: language + theme pickers (DaisyUI dropdowns, auto-persist) + GitHub link, then min/max/close window controls. Scroll is scoped to `<main>` below the titlebar (`h-screen` flex column, `overflow-hidden` root, `overflow-y-auto` main) so scrollbars never pass through the titlebar. `main` has `mr-1.5 mb-1.5` so the scrollbar sits clear of the window-edge resize handles. Drag region uses `data-wails-drag`; Wails v2.13 detects drag via CSS custom property `--wails-draggable: drag` (mapped in `app.css`), NOT the attribute alone. Drag region is non-interactive, double-click toggles maximize. Window controls call Wails runtime: `WindowMinimise`, `WindowToggleMaximise` (+ `WindowIsMaximised` state, subscribed to `wails:maximised`/`wails:unmaximised` events), `Quit`. Root has `dir="ltr"` so controls stay physically top-right in RTL (`ar`).
- Window resize: `frontend/src/lib/components/WindowResizeHandles.svelte` (fixed overlay, 8 edge/corner handles) drives `WindowGetSize`/`WindowGetPosition` + `WindowSetSize`/`WindowSetPosition` with pointer capture, rAF-throttled, hidden while maximized. Wails' built-in edge resize is disabled in `main.js` (`window.wails.flags.enableResize = false`) because it breaks when a DOM scrollbar occupies the window edge (wailsapp/wails#4680). Min size clamped in JS (800x600, sync with `MinWidth`/`MinHeight` in main.go).
...
9. Update all file docs needed like `AGENTS.md` file with any new context or notes that will help future tasks. `README.md` if any public-facing changes for human readability.
...
`GetCampaignConfig` also returns an `mcp` block: `{enabled, host, port, token (always redacted), running, status (running/starting/error/stopped), addr (actual bound host:port), error (last start error)}`.

## MCP Server (AI Agents)

Embedded MCP server (`internal/mcp/`, official `github.com/modelcontextprotocol/go-sdk` v1.7.x) served over streamable HTTP. Config via `mcp` section in `config.yaml` (`enabled`, `host`, `port`, `token`). Endpoint `http://<host>:<port>/mcp`, health at `/mcp/health`.

- Lifecycle: `App` creates the `mcp.Server` once (`reconcileMCP`), `Server.Reconcile()` starts/stops/restarts to match config. Restart is async (`stopAsync`/`restartAsync`) because `http.Server.Shutdown` waits for in-flight requests - calling it synchronously inside the `mecs_set_config` handler would deadlock.
- SDK v1.7 note: `ToolAnnotations.ReadOnlyHint`/`IdempotentHint` are plain `bool`; only `DestructiveHint`/`OpenWorldHint` are `*bool`. No `mcp.Ptr` helper - use local `boolPtr`.
- Handler signature: `func(ctx, req *mcp.CallToolRequest, args Params) (*mcp.CallToolResult, any, error)`; SDK infers JSON schema from `json` tags and converts returned errors to MCP error responses. Tool names use `mecs_` prefix.
- Tools share the global store + `cfgProvider` closure (always reads current `App.defaultConfig`). `buildCampaignRequest(csvRequired)` resolves staged config against global defaults (fillSMTP/fillSES, firstNonEmpty/firstNonZero). Secrets masked in all read output via `mask()` (empty stays empty, non-empty becomes `****`).
- `set_config` calls `config.SavePartial` then `reload` callback (`App.reloadConfig`), which reloads defaults and reconciles the server - so changing `mcp.*` restarts it immediately.
- **UI sync**: `store.GetRevision()` bumps on every store mutation (SetCSV/SetCSVText/SetTemplate/SetConfig/Reset). The 1s `campaign:progress` event carries `revision`; when it changes, CampaignForm re-fetches `GetCampaignConfig()` and mirrors the campaign block (template/config/csv_text) into the form via `syncFromStore`/`applyCampaign`. State `ready` does NOT lock the form (MCP-staged campaigns stay editable); clearing via MCP resets the form.

| Tool | Purpose |
| --- | --- |
| `mecs_prepare_campaign` | Stage partial campaign (CSV text/path, template, provider, worker, log); validated, merges into store |
| `mecs_get_current_campaign` | State + progress + staged template/config (masked) + optional events |
| `mecs_set_config` | Deep-merge partial JSON into config.yaml (global) + reload |
| `mecs_get_config` | Global config, optional `section` filter, secrets masked |
| `mecs_dry_run_campaign` | Render up to 5 sample emails from prepared campaign |
| `mecs_start_campaign` | Start prepared campaign (defaults fill gaps); rejects if not prepared/incomplete/running |
| `mecs_pause_campaign` / `mecs_resume_campaign` | Graceful pause / resume pending only (state-guarded like frontend) |
| `mecs_clear_campaign` | Reset store to idle (rejected while running) |
...
internal/
  ...
  mcp/                     # embedded MCP server (go-sdk streamable HTTP, 9 mecs_* tools)
frontend/
  src/
    lib/
      components/
        CampaignForm.svelte   # orchestrator: state, Wails binding calls, tabs, progress, confirm modal
        TitleBar.svelte       # frameless titlebar: drag region (title + version) + lang/theme/MCP-settings/GitHub + min/max/close (Wails runtime)
        ...
        McpConfig.svelte      # MCP server card: enabled/host/port/token + live running badge (settings dropdown in TitleBar)
        WindowResizeHandles.svelte # 8 edge/corner resize handles (WindowGet/SetSize/Position, hidden when maximized)
    ...
    App.svelte             # layout shell: TitleBar + WindowResizeHandles + CampaignForm (scroll scoped to main)
    main.js
